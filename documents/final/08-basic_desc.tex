...
\begin{table}[H]
    ...
    \resizebox{\linewidth}{!}{%
    \input{../../analysis/tables/num_summary}
}
\end{table}
...
As seen in \cref{fig:host_has_profile_pic-pie}, over 99\% of host with
...
\Cref{fig:host_identity_verified-pie} shows that only 74\% of hosts have had
...
As seen in \cref{fig:host_is_superhost-pie}, only the 19\% of hosts are superhosts.
...
\cref{fig:host_response_rate_cat-pie}, 55\% of hosts have a very high
...
\Cref{fig:host_response_time-bar} shows that the quicker the response, the more
...
the four seasons. In \cref{fig:host_since_season-bar} we can see that Spring/Summer
still have a bigger influx but not as much as we had initially hypothesised.


\figf{host_since_year}{host since year}

In \cref{fig:host_since_year-pie} we can see that except for the first years the
...
As seen in \cref{fig:instant_bookable-bar} there are nearly as many
...
Looking at \cref{fig:neighbourhood_group_cleansed-pie} we can see that
...
In \cref{fig:price-hi_bp-tallat500} we can see that there are some extreme
...
that for example L'Eixample has a mean price of ..., %TODO
while Nou Barris has a mean price of ....  %TODO
