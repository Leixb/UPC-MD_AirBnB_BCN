...
profile <- function() {
   ...
   save_prof_table(pvalk, 'pvalues')
   save_prof_table(pvalues, 'pvalues', 'extra')

}

#dd <- readRDS('data/30-data_cluster.Rda')
dd <- readRDS('data/40-data_clusters.Rda')
FILE_PRE <- 'prof'

dd$cluster <- dd$cluster1
save_prof_plot <- function(p, ...)  save_pdf(p, FILE_PRE, 'c1' , ...)
save_prof_table <- function(p, ...)  save_table(p, FILE_PRE, 'c1' , ...)
profile()

dd$cluster <- dd$cluster2
save_prof_plot <- function(p, ...)  save_pdf(p, FILE_PRE, 'c2' , ...)
save_prof_table <- function(p, ...)  save_table(p, FILE_PRE, 'c2' , ...)
profile()

dd$cluster <- dd$cluster3
