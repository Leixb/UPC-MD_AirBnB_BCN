...
```{r, echo=FALSE, out.width="80%", fig.align = 'center',  warning=FALSE, message=FALSE, results='asis'}
...
for(k in actives){
  ...
  if (!(is.numeric(dd[,k]) || class(dd[,k])=="Date")){
    ...
    p <- ggplot(dd, aes(x=!!var_s, fill=!!var_s)) + geom_bar(width = 0.7) + theme(legend.position = "none", axis.text.x=element_text(angle = -90, hjust = 0))
    print(p)
...
```{r, echo=FALSE}
kbl(table(dd$host_response_rate_cat), booktabs=T)
